...
@external
def safeTransferFrom(_from: address, _to: address, _id: uint256, _amount: uint256, _data: Bytes[CALLBACK_NUMBYTES]):
    """
    """
    assert _from == msg.sender or self.isApprovedForAll(_from, msg.sender), "ERC1155: caller is not token owner or approved"
    self._safeTransferFrom(_from, _to, _id, _amount, _data)



@external
def safeBatchTransferFrom(_from: address, _to: address, _ids: DynArray[uint256, BATCH_SIZE], _amounts: DynArray[uint256, BATCH_SIZE], _data: Bytes[CALLBACK_NUMBYTES]):
    """
    """
    assert _from == msg.sender or self.isApprovedForAll(_from, msg.sender), "ERC1155: caller is not token owner or approved"
    self._safeBatchTransferFrom(_from, _to, _ids, _amounts, _data)



@internal
def _safeTransferFrom(_from: address, _to: address, _id: uint256, _amount: uint256, _data: Bytes[1024]):
    ...
    assert fromBalance >= _amount, "ERC1155: insufficient balance for transfer"
    self._balances[_from][_id] = fromBalance - _amount
    self._balances[_to][_id] += _amount

    log TransferSingle(_operator, _from, _to, _id, _amount)




@internal
def _safeBatchTransferFrom(_from: address, _to: address, _ids: DynArray[uint256, BATCH_SIZE], _amounts: DynArray[uint256, BATCH_SIZE], _data: Bytes[CALLBACK_NUMBYTES]):
    """
    ...
    for i in _ids:
        ...
        self._balances[_from][id] = fromBalance - amount
        self._balances[_to][id] += amount

    log TransferBatch(_operator, _from, _to, _ids, _amounts)

    # self._afterTokenTransfer(operator, from, to, ids, amounts, data)


@internal
def _mint(_to: address, _id: uint256, _amount: uint256, _data: Bytes[CALLBACK_NUMBYTES]): 
    ...
    self._balances[_to][_id] += _amount
    log TransferSingle(_operator, ZERO_ADDRESS, _to, _id, _amount)

    # self._afterTokenTransfer(operator, ZERO_ADDRESS, to, id, amount, data)


@internal
def _mintBatch(_to: address, _ids: DynArray[uint256, BATCH_SIZE], _amounts: DynArray[uint256, BATCH_SIZE], _data: Bytes[CALLBACK_NUMBYTES]):
    assert _to != ZERO_ADDRESS, "ERC1155: mint to the zero address"
    assert len(_ids) == len(_amounts), "ERC1155: ids and amounts length mismatch"
    ...
    for i in _ids:
        self._balances[_to][i] += _amounts[i]

    log TransferBatch(_operator, ZERO_ADDRESS, _to, _ids, _amounts)

@internal
def _burn(_from: address, _id: uint256, _amount: uint256):
    ...
    fromBalance: uint256 = self._balances[_from][_id]
    assert fromBalance >= _amount, "ERC1155: burn amount exceeds balance"
    self._balances[_from][_id] = fromBalance - _amount

    log TransferSingle(_operator, _from, ZERO_ADDRESS, _id, _amount)

    # self._afterTokenTransfer(operator, from, ZERO_ADDRESS, id, amounts, b"")

@internal
def _burnBatch(_from: address, _ids: DynArray[uint256, BATCH_SIZE], _amounts: DynArray[uint256, BATCH_SIZE]):
    assert _from != ZERO_ADDRESS, "ERC1155: burn from the zero address"
    assert len(_ids) == len(_amounts), "ERC1155: ids and amounts length mismatch"
    ...
    for i in _ids:
        ...
        self._balances[_from][_id] = fromBalance - amount

    log TransferBatch(_operator, _from, ZERO_ADDRESS, _ids, _amounts)
